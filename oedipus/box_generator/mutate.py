...
def calculate_percent_children_in_bin(run_id, generation, p_id):
...
def calculate_mutation_strength(run_id, generation, p_id):
...
#        mutation_strength.generation = generation
    parent = session.query(Box).get(p_id)
    try:
        fraction_in_parent_bin = calculate_percent_children_in_bin(run_id, generation, p_id)
        if fraction_in_parent_bin < 0.1 and parent.mutation_strength / 1.1 > 0:
            parent.mutation_strength /= 1.1
        elif fraction_in_parent_bin > 0.5 and parent.mutation_strength + 1.1 < 1:
            parent.mutation_strength *= 1.1
...
#    return mutation_strength.strength
    return parent.mutation_strength
...
def mutate_box(parent_box, mutation_strength, generation, initial_mutation_strength):
    """    
    Args:
    ...
    ########################################################################
    # create box
    child_box = Box(initial_mutation_strength, parent_box.run_id)
...
#            mutation_strength = MutationStrength.get_prior(*mutation_strength_key).clone().strength
            if gen > 1:
                mutation_strength = calculate_mutation_strength(run_id, gen, parent_box.id)
            else:
                mutation_strength = config['initial_mutation_strength']
...
#            ms = MutationStrength.get_prior(*mutation_strength_key).clone().strength
            if gen > 1:
                ms = calculate_mutation_strength(run_id, gen, parent_box.id)
            else:
                ms = config['initial_mutation_strength']
            mutation_strength = np.random.choice([1., ms])
        else:
            print("REVISE CONFIG FILE, UNSUPPORTED MUTATION SCHEME.")
    
        # mutate material
        box = mutate_box(parent_box, mutation_strength, gen, config['initial_mutation_strength'])
        boxes.append(box)
    return boxes
